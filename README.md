...
* 과제로 만든 __Maze Pathfinder__ 입니다. 네. 그냥 미로 길찾기에요. 개인적으로 이것저것 많은 요소가 들어간 프로그램이고, C에서 나름 __모듈화__ 하려고 발광했던 프로젝트라 github에 등록했습니다. 사실 파일 하나로도 짤 수 있었을 텐데 코드가 길어져서.. 분할했습니다 ㅜㅜ. 혹시라도 보시는 분이 있다면 참고라도 되었으면 좋겠네요. 이번에 코드분할 하면서 Makefile 처음 만들어 봤는데 어려웠어요.
* 윈도우와 리눅스의 개행(줄바꿈) 차이(윈도우 - '\r\n', 리눅스 - '\n')가 나타날 수 있습니다. __소스 코드 다운 받아 확인하시는 분들은 주의하세요.__(메모장 말고 Notepad++ 이나 Sublime 같은 친구로 여시는 것을 추천)
...
### 컴파일
* CLI 환경에서 아래 커맨드로 컴파일 가능하다. 리눅스 사용자는 gcc, 윈도우는 아마 mingw 필요
